...
usage() {
  ...
  echo '  -v       Verbose mode. Displays the server name before executing the COMMAND.' >&2
  exit 1
}

# Make sure this script is not being executed with superuser privileges.
if [[ "${UID}" -eq 0 ]]; then
...
# Make sure the SERVER_LIST exists.
